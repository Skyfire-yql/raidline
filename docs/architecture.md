...
完整原始事件、玩家姓名、清洗快照、导入草稿和本地计划产物不会进入 Git 或前端。审查器按事件波次压缩重复 Tick，但保留原始页和每一波的时间、条数、目标数及关联技能；人工决定文件在重新生成时保留。正式调研优先使用官方 WCL v2 API，兼容研究源不得成为正式后端依赖。当前 Vashnik 只注册正式服 encounter `3455` 的 profile v3；其证据来自 9 场公开史诗击杀和同报告 102 次灭团 Pull，仓库不保留该 Boss 的历史 profile 或静态时间轴预设。
