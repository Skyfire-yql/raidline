...
> 当前规则已于 2026-09-08 收敛，见 [最新决定](requirements/2026-09-08-消融后的功能收敛.md) 与 [实施验证](simplification-2026-09-08.md)。仅保留 profile v2；技能使用全局库，取消计划内快照、变体、分组和旧文案兼容，按成功施法的开始时间排轴，使用最短冷却。下文是 2026-09-05 的历史实施与取证记录，冲突描述不再代表当前产品；17 条待核验规则和数值核准门槛仍有效。
