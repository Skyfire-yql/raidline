...
- 当前网站只允许通过服务端只读探针连接真实 WCL，读取公开报告、Boss 战斗和官方阶段；不写快照、不连接数据库/队列。开发者本地可以用 `wcl:download` / `wcl:review` / `wcl:convert` 在被 Git 忽略的 `work/` 研究完整事件并模拟转换，但不得把原始事件、玩家姓名、本地清洗/草稿产物、兼容测试源或凭据放进产品 API、计划、仓库或生产依赖。仓库当前不维护任何托管平台部署配置。
