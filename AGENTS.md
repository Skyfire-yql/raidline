...
- 当前网站只允许通过服务端只读边界连接真实 WCL：读取公开报告、Boss 战斗、官方阶段，并在史诗难度与精确 encounter profile 校验后分页读取该战斗所需事件。导入 API 只能返回匿名机制候选和严格 plan 预览；不得返回原始事件、玩家姓名、敏感头或服务端快照。只有用户显式确认后才在浏览器本地保存所选机制，不连接服务器数据库/队列，也不写分享。开发者仍可用 `wcl:download` / `wcl:review` / `wcl:convert` 在被 Git 忽略的 `work/` 研究完整事件；本地清洗/草稿产物、兼容测试源和凭据不得进入产品 API、计划、仓库或生产依赖。仓库当前不维护任何托管平台部署配置。
