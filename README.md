...
- 单条计划在本地保存和发布前都执行 v4 结构校验，并限制为 1MB；v1/v2/v3 内容会在浏览器中迁移到 v4。
...
测试覆盖 v1/v2/v3→v4、目录 v1→v2、自动时间范围、计划校验、目录引用快照、基础排轴计算、精确 16+4 Base62 ID，以及 R2 创建、读取、错误编辑 ID、覆盖、新链接和删除行为。
