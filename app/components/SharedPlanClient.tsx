...
import { SEED_CATALOG, validateCatalogRelease } from "@/lib/catalog";
...
import type { ApiError, CatalogRelease, PlayerSkillDefinition, MemberSelector, PublicPublication, RaidPlanDocument } from "@/lib/types";
...
import { createLocalPlan, cacheCatalog, getCachedCatalog } from "./local-store";
...
function ReadonlyDetails({ plan, selection, library }: { library: readonly PlayerSkillDefinition[]; plan: RaidPlanDocument; selection: SharedSelection }) {
  ...
  if (assignment) {
    const memberName = plan.roster.members.find((item) => item.id === assignment.memberId)?.name ?? "未知成员"; const skill = resolveSkillForAssignment(assignment, library); const time = resolveTimelineAnchor(plan, assignment.anchor);
    return <div className="inspector readonly-inspector"><header><h2>{skill?.name ?? "未知技能"}</h2><span>{skill ? skillDataStatusLabel(skill.dataStatus) : "技能安排"}</span></header><ReadonlyField label="成员" value={memberName} /><ReadonlyField label="开始时间" value={time.ok ? formatTime(time.atMs) : "无法解析"} />{skill && <ReadonlyField label="技能时间" value={`${skill.cooldownMs == null ? "冷却待补" : `最短冷却 ${skill.cooldownMs / 1000} 秒`} · ${skill.maxCharges} 层充能 · ${skill.castType === "channel" ? "引导" : skill.castType === "cast" ? "读条" : skill.castType === "instant" ? "瞬发" : "施法待补"}`} />}{skill && <ReadonlyField label="技能效果" value={skillEffectText(skill)} multiline />}{assignment.note && <ReadonlyField label="备注" value={assignment.note} multiline />}</div>;
  }
...
export function SharedPlanClient({ shareId }: { shareId: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getCachedCatalog().then(cached => { if (cached && !cancelled) setCatalog(cached); }).catch(() => undefined).then(() => fetch("/api/catalog/current")).then(async response => {
      if (!response.ok) return;
      const payload = await response.json() as { data: CatalogRelease };
      const current = validateCatalogRelease(payload.data);
      await cacheCatalog(current);
      if (!cancelled) setCatalog(current);
    }).catch(() => undefined);
  ...
  const warnings = useMemo(() => plan ? detectConflicts(plan, catalog.playerSkills) : [], [plan, catalog.playerSkills]);
