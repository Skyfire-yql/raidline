...
import type { ApiError, PublicPublication } from "@/lib/types";
...
export function SharedPlanClient({ shareId }: { shareId: string }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (cancelled) return;
      try {
        const view = JSON.parse(localStorage.getItem(viewPreferenceKey("shared", shareId, innerWidth)) ?? "null") as { orientation?: TimelineOrientation; zoom?: number } | null;
  ...
  useEffect(() => {
    if (viewReady) localStorage.setItem(viewPreferenceKey("shared", shareId, viewWidth), JSON.stringify({ orientation, zoom }));
  }, [orientation, shareId, viewReady, viewWidth, zoom]);
  ...
  return (
    <main className="shared-workspace">
      <header className="utility-header shared-utility-header">
        ...
        <div><span className="readonly-state">只读</span><button onClick={async () => { const local = await createLocalPlan(plan); router.push(`/plans/${local.id}`); }}>复制到我的轴</button><button onClick={async () => { await navigator.clipboard.writeText(exportMrtNote(plan)); setToast("MRT 文本已复制"); }}>复制 MRT</button><ThemeControl compact /></div>
      </header>
      <section className="shared-summary">
        <div><h1>{plan.encounter.name}</h1><p>{plan.encounter.difficulty} · {formatTime(plan.encounter.durationMs)} · 发布于 {new Date(stored.publishedAt).toLocaleString("zh-CN")}</p></div>
