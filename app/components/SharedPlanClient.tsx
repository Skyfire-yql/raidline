...
export function SharedPlanClient({ shareId }: { shareId: string }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => { if (cancelled) return; try { const view = JSON.parse(localStorage.getItem(viewPreferenceKey("shared", shareId, innerWidth)) ?? "null") as { orientation?: TimelineOrientation; zoom?: number; leftCollapsed?: boolean } | null; setViewWidth(innerWidth); setOrientation(view?.orientation ?? defaultOrientation(innerWidth)); setZoom(Math.max(1, view?.zoom ?? 1)); setLeftCollapsed(view?.leftCollapsed ?? innerWidth < 720); } catch { setViewWidth(innerWidth); setOrientation(defaultOrientation(innerWidth)); setLeftCollapsed(innerWidth < 720); } setViewReady(true); });
    return () => { cancelled = true; };
  }, [shareId]);
  useEffect(() => { if (viewReady) localStorage.setItem(viewPreferenceKey("shared", shareId, viewWidth), JSON.stringify({ orientation, zoom, leftCollapsed })); }, [leftCollapsed, orientation, shareId, viewReady, viewWidth, zoom]);
  useEffect(() => {
