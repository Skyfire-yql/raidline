...
import { parseCatalogRelease, parsePlanDocument, type CatalogMechanicDefinition, type CatalogRelease, type CatalogSkillDefinition, type MechanicDefinitionSnapshot, type RaidPlanDocument, type TimelinePreset } from "./types";
...
function validateVerification(skill: CatalogSkillDefinition) {
  ...
  if (duplicateValues(skill.observedSpells.map(item => String(item.spellId))).length) throw new Error(`技能 ${skill.name} 的观测法术 ID 重复`);
...
function builtInCatalog() {
  ...
  release.manifest.version = "builtin-seed-v7";
  ...
  release.playerSkills = structuredClone(PLAYER_SKILLS);
