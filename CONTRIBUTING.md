...
感谢参与 Raidline。项目目前处于 vNext v1 的基础模型阶段，核心编辑体验已经稳定，并已有只读 WCL 报告/战斗/阶段探针、Vashnik 正式服匿名导入、跨样本目录骨架和开发者本地完整事件研究工具；快照持久化、玩家技能转换、对比分析、生产服务器存储和更多导出器仍是后续里程碑。贡献时请优先保持领域模型清楚、边界可替换和现有双栏交互稳定。
