...
感谢参与 Raidline。项目目前处于 vNext v1 的基础模型阶段，核心编辑体验已经稳定，并已有只读 WCL 报告/战斗/阶段探针；WCL 事件采集、快照持久化、技能转换、对比分析、生产服务器存储和更多导出器仍是后续里程碑。贡献时请优先保持领域模型清楚、边界可替换和现有双栏交互稳定。
...
本地页面默认由 Vinext 提供。当前 Sites 绑定只用于临时交互验证，不要把正式 WCL 凭据、生产数据或唯一备份上传到 Sites。

真实 WCL 探针使用根目录 `.env.local` 中的 `WCL_CLIENT_ID` 与 `WCL_CLIENT_SECRET`。该文件已经被 Git 忽略；凭据只能由服务端 `cloudflare:workers` 环境读取，禁止通过 `VITE_`、`NEXT_PUBLIC_`、页面 props、日志或 fixture 暴露。
...
- 计划、目录、WCL 快照和导出契约必须保持提供者无关。不要把 React、IndexedDB、SQLite、R2、WCL GraphQL DTO 或插件语法放进领域 schema。
...
- 对象存储通过 `ObjectStore` 接口访问；不要在业务逻辑中扩散 Cloudflare 专有 API。
- 现有首页、双栏编辑器、拖拽、缩放、主题、只读分享和 MRT 阅读版是稳定工作流。视觉调整要有对应的浏览器回归验证。

### WCL 与导出

- 当前阶段只维护提供者无关的 WCL 契约和 fixture，不发起真实 WCL 请求。
